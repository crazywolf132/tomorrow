    ...
    function patch(fs2) {
      ...
      function statFix(orig) {
        ...
        return function(target2, options, cb) {
          ...
          return options ? orig.call(fs2, target2, options, callback) : orig.call(fs2, target2, callback);
        };
      }
      function statFixSync(orig) {
        ...
        return function(target2, options) {
          var stats = options ? orig.call(fs2, target2, options) : orig.call(fs2, target2);
          if (stats) {
            if (stats.uid < 0)
              stats.uid += 4294967296;
            if (stats.gid < 0)
...
// package.json
var settings = {
  testRange: "../../../testRange/tomorrow/",
  target: "es2015",
  format: "cjs"
};

// src/tsup.ts
var { target, testRange, format } = settings;
var cloneToTestRange = (pkgName, fileName) => {
  import_fs_extra.default.copyFileSync(import_path.default.join(process.cwd(), "dist", fileName), import_path.default.resolve(testRange, "node_modules", pkgName, "dist", fileName));
};
var tsupBuilder = (settings2) => {
  var _a;
  const cwd = process.cwd();
  const pkg = import_fs_extra.default.readJSONSync(import_path.default.join(cwd, "package.json"));
  const { name } = pkg;
  if (settings2.target) {
  ...
  const config = __spreadProps(__spreadValues({}, settings2), {
    target,
    external: [...(_a = settings2.external) != null ? _a : [], settings2.reactNative ? "react-native" : ""]
  });
  if (settings2.cloneFile) {
    config.onSuccess = () => {
      var _a2;
      ((_a2 = settings2.entry) != null ? _a2 : []).forEach((entry) => {
