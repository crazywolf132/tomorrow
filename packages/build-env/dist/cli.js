...
// src/commands/createPackage.ts
var import_fs_extra = __toESM(require_lib());
var import_path = require("path");
var import_inquirer = __toESM(require("inquirer"));

// template.package.json
var template_package_default = {
  name: "",
  version: "0.0.1",
  main: "./dist/index.js",
...
var createPackage_default = {
  ...
  action: () => __async(void 0, null, function* () {
    ...
    prompt([
      ...
      const modifiedTemplate = __spreadProps(__spreadValues({}, template_package_default), {
        name: answers.tomorrow ? `@tomorrow/${answers.name}` : answers.name,
        peerDependencies: __spreadValues(__spreadValues({}, template_package_default.peerDependencies), answers.native ? { "react-native": "*" } : {}),
        devDependencies: __spreadValues(__spreadValues({}, template_package_default.devDependencies), answers.native ? { "react-native": "latest", "@types/react-native": "latest", "react": "latest", "@types/react": "latest" } : { "@types/node": "latest" })
      });
      const location = (0, import_path.join)(process.cwd(), "packages", answers.tomorrow ? `@tomorrow/${answers.name}` : answers.name);
      import_fs_extra.default.ensureFileSync((0, import_path.join)(location, "package.json"));
      import_fs_extra.default.ensureFileSync((0, import_path.join)(location, "src", "index.ts"));
      import_fs_extra.default.ensureFileSync((0, import_path.join)(location, "tsup.config.ts"));
      import_fs_extra.default.writeFileSync((0, import_path.join)(location, "package.json"), JSON.stringify(modifiedTemplate, null, 4));
      import_fs_extra.default.writeFileSync((0, import_path.join)(location, "tsup.config.ts"), tsupTemplate.replace("{{native}}", answers.native ? "true" : "false"));
      if (!answers.native) {
        import_fs_extra.default.writeFileSync((0, import_path.join)(location, "tsconfig.json"), JSON.stringify(nodeTsConfig, null, 4));
      }
    });
  })
};
...
var commands_default = [createPackage_default];

// src/cli.ts
var program2 = new Command();
program2.name("BUILD-ENV").description("Local package helper").version(version);
commands_default.forEach((command) => {
  var _a, _b;
  let temp = program2.command(command.name);
