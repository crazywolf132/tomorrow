...
// Filter files. Removes all files we don't care to map
export function checkValidFileName(fileName: string): boolean {
    return /(page|layout|template|loading|error)\.[jt]sx?$/.test(fileName)
}
