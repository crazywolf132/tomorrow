...
var require_dist = __commonJS({
  "../../debug/dist/index.js"(exports, module2) {
    ...
    var import_ms = __toESM2(require_ms());
...
var init_default = {
  name: "init",
  description: "adds the required packages to the project",
  action: () => __async(void 0, null, function* () {
  })
};

// src/commands/index.ts
var commands_default = [init_default];

// src/index.ts
var debug = import_debug.default.get("cli");
var program = new import_commander.Command();
program.name("BUILD-ENV").description("Local package helper").version(version);
debug("initialized");
commands_default.forEach((command) => {
  var _a, _b;
  debug("added command", command.name);
  let temp = program.command(command.name);
  for (const option of (_a = command.options) != null ? _a : []) {
    let [flags, description, defaultValue] = option;
    temp.option(flags, description, defaultValue);
  }
  for (const argument of (_b = command.arguments) != null ? _b : []) {
